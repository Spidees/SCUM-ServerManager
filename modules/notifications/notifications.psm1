...
$CommonModulePath = Join-Path (Split-Path $PSScriptRoot -Parent) "common\common.psm1"
...
function Notify-ServerStatusChange {
    ...
    switch ($NewStatus) {
        "Online" {
            if ($PreviousStatus -ne "Online") {
                Send-Notification -Type "admin" -MessageKey "serverStarted" -Vars $Context
                Send-Notification -Type "player" -MessageKey "serverOnline" -Vars $Context
            }
        }
        "Offline" {
            if ($PreviousStatus -eq "Online") {
                Send-Notification -Type "admin" -MessageKey "serverStopped" -Vars $Context
                Send-Notification -Type "player" -MessageKey "serverOffline" -Vars $Context
            }
        }
    }
