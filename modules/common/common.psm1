...
function Initialize-ConfigPaths {
    ...
    $script:ConfigPaths = @{
        savedDir = Resolve-ConfigPath -Path $Config.savedDir
        backupRoot = Resolve-ConfigPath -Path $Config.backupRoot
        steamCmd = Resolve-ConfigPath -Path $Config.steamCmd
        serverDir = Resolve-ConfigPath -Path $Config.serverDir
...
function Write-Log {
    ...
    # Output to file if path is configured
    if ($script:logPath) {
        try {
